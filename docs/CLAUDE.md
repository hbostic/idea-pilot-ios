...
7. **Documentation check**: Verify platform-specific documentation is complete:
   - Swagger/OpenAPI annotations for any new or modified endpoints
   - JSDoc/TSDoc for all new public functions, services, and exported utilities
   - Inline comments for business rules, non-obvious logic, and workarounds
   - README or setup docs updated if configuration, env vars, or dependencies changed
   - Architecture decision records (ADRs) created for significant design choices
8. **Production readiness check**: Review changes against known production readiness concerns:
